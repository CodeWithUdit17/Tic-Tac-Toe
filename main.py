...
class TicTacToeApp:
    ...
    def build_ui(self) -> None:
        ...
        board_container = tk.Frame(self.root, bg=self.theme["grid"], padx=3, pady=3)
        ...
        tk.Button(action_frame, text="Save XML", bg="#DCFCE7", command=self.save_game, **btn_style).grid(row=0, column=0, padx=5, pady=5)
        tk.Button(action_frame, text="Load XML", bg="#DBEAFE", command=self.load_game, **btn_style).grid(row=0, column=1, padx=5, pady=5)
        tk.Button(action_frame, text="Undo", bg="#FEF3C7", command=self.undo_move, **btn_style).grid(row=0, column=2, padx=5, pady=5)
        tk.Button(action_frame, text="New Round", bg="#F1F5F9", command=self.new_round, **btn_style).grid(row=0, column=3, padx=5, pady=5)

    def on_cell_clicked(self, row: int, col: int) -> None:
        """Handles user clicking on a board tile with instant 0ms response."""
        ...
        if self.engine.game_mode == "PvE" and self.engine.current_player == "O":
            self.root.after(35, self.make_ai_move)

    def make_ai_move(self) -> None:
        """Executes computer opponent turn with instant 0ms calculation."""
        if self.engine.is_game_over:
            return
